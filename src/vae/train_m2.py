...
def generate_image(net, x, t, file_path):
    y = net(x, t)
    im_x = cuda.to_cpu(x)
    im_x = im_x.reshape((280, 28)).transpose()
    im_y = cuda.to_cpu(y.data).transpose((0, 2, 1, 3)).reshape((280, 280))
    im = np.concat(im_x, im_y, axis=1)
...
def train(net, optimizer, iterator, iteration, test_data, name):
    xp = net.xp
    test_x, test_t = convert.concat_examples(batch[:10])
    test_x = xp.asarray(test_x)
    test_t = xp.asarary(test_t)
